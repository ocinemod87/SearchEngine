...
class IndexTest {
	...
	/**
	 * Check if it is possible to put things into the inverted indices without using the build command.
	 */
...
//	}
     ...
     */
     private void lookupInEmptyIndex(Index index) {
 		...
 		assertTrue(index.lookup("anyword") instanceof Collection, () -> "lookup() should always return a collection");
 	}
	...
	/**
	 * Test if lookup method returns something not null independent of whether it can find the word or not.
	 * Also test if the collection returned has the correct number of websites. 
	 */
    private void lookup(Index index) {
    	...
    	assertAll("check return type",
    			() -> { assertNotNull(index.lookup("word1"));
    	    		  	...
    	    		  	assertTrue(index.lookup("word1") instanceof Collection, "lookup must return a collection of websites");
